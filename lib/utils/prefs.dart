...
Future<dynamic> getColor({int? colorNum}) async {
  final prefs = await SharedPreferences.getInstance();
  return colorNum == null
      ? prefs.getInt("colorNum") ?? 0
      : await prefs.setInt("colorNum", colorNum);
}

Future<bool> getIsDark({bool? isDark}) async {
  final prefs = await SharedPreferences.getInstance();
  return isDark == null ? prefs.getBool("isDark") ?? false : await prefs.setBool("isDark", isDark);
}
